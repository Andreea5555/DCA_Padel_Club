...
public class AddCourtEndpoint(ICommandDispatcher dispatcher)
    : ApiEndpoint
        .WithRequest<AddCourtRequest>
        .AndResults<NoContent, BadRequest<IReadOnlyList<OperationError>>>
{
    [HttpPost("schedules/{id}/add-court")]
    public override async Task<Results<NoContent, BadRequest<IReadOnlyList<OperationError>>>>
        HandleAsync([FromRoute] AddCourtRequest request)
    {
        Result<AddCourtCommand> commandResult =
            AddCourtCommand.Create(request.Id, request.RequestBody.CourtId);
...
public record AddCourtRequest(
    ...
    [FromBody] AddCourtRequest.Body RequestBody)
{
