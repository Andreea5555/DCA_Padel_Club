...
public class RemoveCourtEndpoint(ICommandDispatcher dispatcher)
    : ApiEndpoint
        .WithRequest<RemoveCourtRequest>
        .AndResults<NoContent, BadRequest<IReadOnlyList<OperationError>>>
{
    [HttpPost("schedules/{id}/remove-court")]
    public override async Task<Results<NoContent, BadRequest<IReadOnlyList<OperationError>>>>
        HandleAsync([FromRoute] RemoveCourtRequest request)
    {
        Result<RemoveCourtCommand> commandResult =
            RemoveCourtCommand.Create(request.Id, request.RequestBody.CourtId);
...
public record RemoveCourtRequest(
    [FromRoute(Name = "id")] string Id,
    [FromBody] RemoveCourtRequest.Body RequestBody)
{
    public record Body(string CourtId);
}