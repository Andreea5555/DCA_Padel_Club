...
public class UpdateScheduleTimesHandlerTest
{
    [Fact]
    public async Task Handle_ValidInput_ReturnsSuccess()
    {
        ...
        Result<UpdateScheduleTimeCommand> commandResult =
            UpdateScheduleTimeCommand.Create(
                schedule.Id.Id.ToString(),
                "15:00",
                "16:00");

        UpdateScheduleTimeCommand command = commandResult.value;

        ICommandHandler<UpdateScheduleTimeCommand> handler =
            new UpdateScheduleTimeHandler(
                repository,
                unitOfWork);
