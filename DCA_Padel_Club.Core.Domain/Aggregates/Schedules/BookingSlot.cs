...
public class BookingSlot : ValueObject
{
    ...
    private static List<OperationError> Validate(TimeOnly startTime, TimeOnly endTime)
    {
        ...
        if (startTime.Minute != 0 && startTime.Minute != 30)
            errors.Add(
                OperationError.Create(
                    "BookingSlot.Start.InvalidMinutes",
        ...
        if (endTime.Minute != 0 && endTime.Minute != 30)
            errors.Add(
                OperationError.Create(
                    "BookingSlot.End.InvalidMinutes",
    ...
    internal List<OperationError> ValidateFitsWithin(
