...
public class Schedule : AggregateRoot<ScheduleId>
{
    ...
    public Result<None> UpdateScheduledTimes(TimeOnly startTime, TimeOnly endTime)
    {
        ...
        if (startTime > endTime)
        {
            errors.Add(
                OperationError.Create(
                    "Schedule.StartTimeTooBig",
                    "The start time cannot be greater than the end time of the schedule."
                )
            );
        }
        ...
        if (
            startTime.Minute != 30 && startTime.Minute != 0
            || endTime.Minute != 30 && endTime.Minute != 0
        )
        {
            errors.Add(
                OperationError.Create(
                    "Schedule.InvalidMinute",
                    "The minutes of the start time and the end time must be either half or whole hours"
                )
            );
        }
        if (errors.Count > 0)
    ...
    public Result<None> ActivateSchedule(
        IActiveScheduleOnDate activeScheduleOnDate,
